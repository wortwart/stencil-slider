...
const locationUtils = require('./location-utils-555418b1.js');
...
const createBrowserHistory = (win, props = {}) => {
    ...
    const basename = props.basename ? locationUtils.stripTrailingSlash(locationUtils.addLeadingSlash(props.basename)) : '';
    ...
    const getDOMLocation = (historyState) => {
        ...
        warning((!basename || locationUtils.hasBasename(path, basename)), 'You are attempting to use a basename on a page whose URL path does not begin ' +
            'with the basename. Expected path "' + path + '" to begin with "' + basename + '".');
        if (basename) {
            path = locationUtils.stripBasename(path, basename);
        }
        return locationUtils.createLocation(path, state, key || locationUtils.createKey(keyLength));
    };
    ...
    const createHref = (location) => {
        return basename + locationUtils.createPath(location);
    };
    const push = (path, state) => {
        warning(!(typeof path === 'object' && path.state !== undefined && state !== undefined), 'You should avoid providing a 2nd state argument to push when the 1st ' +
            'argument is a location-like object that already has state; it is ignored');
        const action = 'PUSH';
        const location = locationUtils.createLocation(path, state, locationUtils.createKey(keyLength), history.location);
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            if (!ok) {
                return;
            }
...
const HashPathCoders = {
    hashbang: {
        encodePath: (path) => path.charAt(0) === '!' ? path : '!/' + locationUtils.stripLeadingSlash(path),
        decodePath: (path) => path.charAt(0) === '!' ? path.substr(1) : path
    },
    noslash: {
        encodePath: locationUtils.stripLeadingSlash,
        decodePath: locationUtils.addLeadingSlash
    },
    slash: {
        encodePath: locationUtils.addLeadingSlash,
        decodePath: locationUtils.addLeadingSlash
    }
};
const createHashHistory = (win, props = {}) => {
    ...
    const getDOMLocation = () => {
        ...
        return locationUtils.createLocation(path, undefined, locationUtils.createKey(keyLength));
    };
    const transitionManager = createTransitionManager();
    const setState = (nextState) => {
        Object.assign(history, nextState);
    ...
    const handleHashChange = () => {
        ...
        if (path !== encodedPath) {
            ...
            if (!forceNextPop && locationUtils.locationsAreEqual(prevLocation, location)) {
                return; // A hashchange doesn't always == location change.
            }
            if (ignorePath === locationUtils.createPath(location)) {
    ...
    const revertPop = (fromLocation) => {
        ...
        let toIndex = allPaths.lastIndexOf(locationUtils.createPath(toLocation));
        let fromIndex = allPaths.lastIndexOf(locationUtils.createPath(fromLocation));
        if (toIndex === -1) {
            toIndex = 0;
        }
        if (fromIndex === -1) {
    ...
    let allPaths = [locationUtils.createPath(initialLocation)];
    // Public interface
    const createHref = (location) => ('#' + encodePath(basename + locationUtils.createPath(location)));
    const push = (path, state) => {
        warning(state === undefined, 'Hash history cannot push state; it is ignored');
        const action = 'PUSH';
        const location = locationUtils.createLocation(path, undefined, locationUtils.createKey(keyLength), history.location);
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            if (!ok) {
                return;
            }
            const path = locationUtils.createPath(location);
            ...
            if (hashChanged) {
                ...
                const prevIndex = allPaths.lastIndexOf(locationUtils.createPath(history.location));
    ...
    const replace = (path, state) => {
        ...
        transitionManager.confirmTransitionTo(location, action, getUserConfirmation, (ok) => {
            ...
            const prevIndex = allPaths.indexOf(locationUtils.createPath(history.location));
            if (prevIndex !== -1) {
